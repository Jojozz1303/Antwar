(function(Scratch) {
  ...
  class AntwarExtension {
    ...
    getInfo() {
      return {
        ...
        blocks: [
          ...
          {
            opcode: 'compareAntEnergy',
            blockType: Scratch.BlockType.BOOLEAN,
            text: 'énergie moyenne [OP] [VALUE]',
            arguments: {
              OP: {
                type: Scratch.ArgumentType.STRING,
                menu: 'operators',
    ...
    setSimulationSpeed(args) {
      this.simulationSpeed = Math.max(0.5, Math.min(5, parseFloat(args.SPEED)));
    }
    ...
    toggleSimulation(args) {
      this.isRunning = args.STATE === 'play';
    }
    ...
    compareAntEnergy(args) {
      const energy = this.getAntEnergy();
      const value = parseFloat(args.VALUE);
      const op = args.OP;

      switch (op) {
        case '=': return energy === value;
        case '>': return energy > value;
        case '<': return energy < value;
        case '≥': return energy >= value;
        case '≤': return energy <= value;
